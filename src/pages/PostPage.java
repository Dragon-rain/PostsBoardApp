...
public class PostPage extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		 ...
		 * response.setCharacterEncoding("UTF-8"); String head =
		 * "<head><title>Post</title><link rel='stylesheet' type='text/css' href='" +
		 * cssLocation + "'></head>"; out.println("<html/>"); out.println(head);
		 * out.print(Header.GetHeader()); out.println("<h1>"+post.getTitle()+"</h1>");
		 * out.println("<p>"+format.format(post.getCreated())+"</p>");
		 * out.println("<p>"+post.getDescription()+"</p>");
		 * if(user!=null&&user.getId()==post.getUserId()){
		 * out.print("<a href='/ProjectTest1/edit-post?id="+post.getId()
		 * +"'><button>편집 </button></a>");
		 * out.print("<form action='delete-post' method='post'><input name='id' value='"
		 * +post.getId()+"' hidden='true'/><button>삭제</button></form>"); }
		 * out.print("</body></html>");
		 */
	}
